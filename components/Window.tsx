...
const Window: React.FC<WindowProps> = ({ instance, app, isMobile, onClose, onMinimize, onMaximize, onFocus, onMove, onResize }) => {
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
    onFocus(instance.id);
    if (isMobile || instance.isMaximized) return; // No drag on mobile
    setIsDragging(true);
  ...
  }, [instance.id, instance.x, instance.y, instance.isMaximized, isMobile, onFocus]);
  ...
  const handleResizeMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>, direction: string) => {
    e.stopPropagation();
    onFocus(instance.id);
    if (isMobile || instance.isMaximized) return; // No resize on mobile
    ...
    if (e.type === 'mousedown') e.preventDefault();
  }, [instance.id, instance.width, instance.height, onFocus, instance.isMaximized, isMobile]);

  const handleDoubleClick = useCallback(() => {
    if (!isMobile) onMaximize(instance.id);
  }, [instance.id, onMaximize, isMobile]);

  useEffect(() => {
    if (isMobile) return; // Skip drag/resize listeners on mobile
    ...
    const handleMouseUp = (e: MouseEvent | TouchEvent) => {
      if (isDragging) {
        ...
        const availableHeight = innerHeight - 48;
  ...
  }, [isDragging, isResizing, instance.id, instance.x, instance.y, isMobile, onMove, onResize, onMaximize]);
  ...
  // Title bar and button sizing
  const headerHeight = isMobile ? 'h-10' : 'h-8';
  const btnSize = isMobile ? 'w-8 h-8' : 'w-6 h-6';
  const iconSize = isMobile ? 'w-5 h-5' : 'w-4 h-4';

  const style: React.CSSProperties = (instance.isMaximized || isMobile) ? {
    top: 0,
    left: 0,
    width: '100%',
    height: isMobile ? 'calc(100% - 56px)' : 'calc(100% - 48px)',
  ...
  return (
    <div
      ref={windowRef}
      className={`absolute flex flex-col bg-slate-700 border border-slate-500/50 ${isMobile ? '' : 'rounded-lg'} shadow-2xl shadow-black/50 overflow-hidden ${visibilityClasses} ${isDragging || isResizing ? '' : transitionClasses}`}
      style={style}
      onMouseDown={() => onFocus(instance.id)}
    >
      <header
        className={`flex items-center justify-between bg-slate-800/80 backdrop-blur-sm text-white ${headerHeight} px-2 select-none ${isMobile ? '' : 'cursor-grab active:cursor-grabbing'} flex-shrink-0`}
        onMouseDown={!isMobile ? handleMouseDown : undefined}
        onTouchStart={!isMobile ? handleMouseDown : undefined}
        onDoubleClick={handleDoubleClick}
      >
        <div className="flex items-center gap-2">
          <div className={isMobile ? 'w-6 h-6' : 'w-5 h-5'}>{app.icon}</div>
          <span className={`font-semibold ${isMobile ? 'text-base' : 'text-sm'}`}>{app.name}</span>
        </div>
        <div className="flex items-center space-x-1">
          {!isMobile && (
            <>
              <button onClick={() => onMinimize(instance.id)} className={`${btnSize} rounded flex items-center justify-center hover:bg-slate-600 transition-colors`}>
                <MinimizeIcon className={`${iconSize} text-white`} />
              </button>
              <button onClick={() => onMaximize(instance.id)} className={`${btnSize} rounded flex items-center justify-center hover:bg-slate-600 transition-colors`}>
                {instance.isMaximized ? <RestoreIcon className={`${iconSize} text-white`} /> : <MaximizeIcon className={`${iconSize} text-white`} />}
              </button>
            </>
          )}
          <button onClick={() => onClose(instance.id)} className={`${btnSize} rounded flex items-center justify-center hover:bg-red-500 transition-colors`}>
            <CloseIcon className={`${iconSize} text-white`} />
          </button>
        </div>
      </header>
